...
if hasattr(st, "fragment"):
    ...
    def run_dashboard_fragment():
        ...
        # If the user toggled pause, we display the last snapshot
        if pause_refresh:
            if "static_df" not in st.session_state:
                st.session_state.static_df = df
                st.session_state.static_state = state
            df = st.session_state.static_df
            state = st.session_state.static_state
        else:
            if "static_df" in st.session_state:
                del st.session_state.static_df
                del st.session_state.static_state
    ...
    # Rerun logic at the end of execution
    if not pause_refresh:
        time.sleep(refresh_interval)
        try:
            st.rerun()
